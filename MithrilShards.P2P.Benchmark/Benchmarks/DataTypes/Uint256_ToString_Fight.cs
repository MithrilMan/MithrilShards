...
namespace MithrilShards.Network.Benchmark.Benchmarks.DataTypes {
   ...
   public class Uint256_ToString_Fight {
      ...
      public void UInt256_BigInteger_FromBytes() {
         for (int i = 0; i < this.N; i++) {
            _ = new BigInteger(this.data[i]).ToString();
         }
      }
      ...
      public void UInt256_Neo_FromBytes() {
         for (int i = 0; i < this.N; i++) {
            _ = new P2P.Benchmark.Benchmarks.DataTypes.Neo.UInt256(this.data[i]).ToString();
         }
      }
      ...
      public void uint256_NBitcoin_FromBytes() {
         for (int i = 0; i < this.N; i++) {
            _ = new uint256(new ReadOnlySpan<byte>(this.data[i])).ToString();
         }
      }
      ...
      public void uint256_MithrilShards_FromBytes() {
         for (int i = 0; i < this.N; i++) {
            _ = new P2P.Benchmark.Benchmarks.DataTypes.MithrilShards.UInt256(this.data[i]).ToString();
         }
      }
      ...
      public void uint256_MithrilShards4Longs_FromBytes() {
         for (int i = 0; i < this.N; i++) {
            _ = new P2P.Benchmark.Benchmarks.DataTypes.MithrilShards.UInt256As4Long(this.data[i]).ToString();
         }
      }
   }
}
